...
```toml
nova = { git = "ssh://git@github.com/xarantolus/nova-lang.git", default-features = false }
```

If you want to print errors returned from the interpreter, enable the `detailed_errors` (or just remove `default-features = false`).
